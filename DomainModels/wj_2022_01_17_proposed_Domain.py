# -*- coding: UTF-8 -*-
"""
@file:wj_2022_01_17_proposed_Domain.py
@author: Wei Jie
@date: 2022/1/17
@description:
"""
...
class inno_model1(nn.Module):
    ...
    def forward(self, x):  # 16,5,3,224,40  b,l,c,frame,feature
        ...
        dom = self.domainFC(attn_out)
        dom = dom.view(-1, self.length, 1)
        dom = torch.transpose(dom, 1, 2)
        dom = self.score_avg(dom).squeeze(2)


        return cla_out,dom,attn_out.view(self.real_batch,self.length,-1),cla
